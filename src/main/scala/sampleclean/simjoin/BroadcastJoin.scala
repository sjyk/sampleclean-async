  ...
  @Override
    ...
    if (!blocker.canPrefixFilter) {
      ...
      if (weighted) {
        ...
        val tokenRankMap: Map[String, Int] = computeTokenCount(smallTableWithId.map(_._2._1)).toSeq
              ...
              filtered.foldLeft(List[Long]()) {
                ...
                case id2 =>
                  ...
                  if ((id2 >= id1) && selfJoin) (null, null, false)
                    ...
                    val similar: Boolean = blocker.similarity(key1, key2, blocker.threshold, weightsValue)
                    (key2, row2, similar)
                  }
              }.withFilter(_._3).map {
