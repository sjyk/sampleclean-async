...
class ParametricOutlier(scc: SampleCleanContext) {
	...
	def getMean(rdd:SchemaRDD):Double = {
		return rdd.map( x => x(1).asInstanceOf[Int].toDouble ).reduce(_ + _)/rdd.count()
	}

	def getSTD(rdd:SchemaRDD, mean:Double):Double = {
		return Math.sqrt(rdd.map( x => Math.pow(x(1).asInstanceOf[Int].toDouble-mean,2) ).reduce(_ + _)/rdd.count())
	}
	...
	def clean(tableName:String, attr: String, z_thresh:Double)={
		...
		val filtered_rdd = sampleToClean.where(x => Math.abs(x(1).asInstanceOf[Int].toDouble - mean)/std <= z_thresh)
