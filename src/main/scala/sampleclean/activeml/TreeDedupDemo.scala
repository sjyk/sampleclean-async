...
object TreeDedupDemo {
  ...
  val taskParameters = CrowdTaskConfiguration(
      maxPointsPerTask = 20, // get 5 labels from each crowd task.
  ...
  val randomForestParameters = RandomForestParameters() // Use defaults
  val frameworkParameters = ActiveLearningParameters(budget=60, batchSize=10, bootstrapSize=10)
  val activeLearningAlgorithm = new UncertaintySamplingRandomForest[DeduplicationPointLabelingContext, DeduplicationGroupLabelingContext]()
  val pointSelector = new ForestUncertaintyFilter[DeduplicationPointLabelingContext]()
...
//  val throwawayGroupContext : GroupLabelingContext = new SentimentGroupLabelingContext("sa", Map())
   ...
   */
  def processNewModel(model:RandomForestModel, modelN: Long) {
    // train a model w/out active learning for comparison
    val passive_model = RandomForest.trainClassifier(trainingData.sample(withReplacement=false, fraction=modelN.toDouble/trainN).map(p => p._2),
      randomForestParameters.numClasses, randomForestParameters.categoricalFeatureInfo, randomForestParameters.numTrees,
      randomForestParameters.featureSubsetStrategy, randomForestParameters.impurity, randomForestParameters.maxDepth,
      randomForestParameters.maxBins)
